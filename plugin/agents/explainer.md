...
2. **Write the checker** into `<skill_dir>/checks/<name>.py`: `python3 check.py <stage_output>`; print JSON `{"fired": bool, "message": "..."}` and exit 1 when fired. It must fire on this round's data and NOT fire on `history/baseline/work/` outputs — test both with Bash before adding it.
